...
ggplot(data=score_gap, aes(x=fct_reorder(name, math), y=math)) +
  ...
  coord_flip() + ggtitle("OECD Maths Performance Score Gap between Genders") +
  xlab("Country") + ylab("Gender gap") + ylim(c(-35, 35))
```

  The math gender gap varies widely in between countries with an unweighted average gap of 4.8 points. The gender gap in maths appears to be wider in Europe and South America in favour of boys and wider in favour of girls in Middle East and the Caribbean. To put it in context, Australia performed relatively well in achieving a small gender gap compared to other countries.
...
rates_sub <- rates %>%
  ...
  ggplot(rates_sub_long, aes(x=date, y=std_r8s, colour=currency)) + geom_line() + ggtitle("Exchange Rates relative to USD") +xlab("Month")+ylab("Crossrate")
