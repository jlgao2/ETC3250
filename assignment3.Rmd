...
1. *About the data*: The chocolates data was compiled by students in a previous class of Prof Cook, by collecting nutrition information on the chocolates as listed on their internet sites. All numbers were normalised to be equivalent to a 100g serving. Units of measurement are listed in the variable name.
  ...
  Using the tour we find that the milk and dark chocolates diverge on most nutritional variables and converge on others. Furthermore there is overlap on all the variables betweek milk and white chocolates. 
...
```
  ...
  It does appear that the types of chocolates generally differ on many variables. The level of sodium and carbs including sugar, and cholestrol are higher in milk chocolates, higher sugar content may be due to the fact that milk chocolates are sweetened more, and the sodium could be due to the addition of milk, which contains a certain level of sodium. Levels of fat and fiber are higher in dark chocolates, due to the increased volume of cocoa mass which has a high fiber content, and increased volume of cocoa butter, which is high in both saturated fat and total fat[1]. 
```{r eval=FALSE}
...
Using the variables of sodium and fiber which provides one plane with clear separation, we find that "Mars Dark Chocolate Bar" sits squarely in the what appears to be the milk chocolate region from visual inspection. We can confirm our guess in questions below.  
```{r eval=FALSE}
...
```
 ...
 $$G = \sum_{k =1}^K \hat{p}_{mk}(1 - \hat{p}_{mk})$$ [2]
...
Intuitively this makes sense, when the Gini function is at 0.5, $\hat{p}_m$ is at 50%, which means that in the split region there is an even mix of classes and the classifer does not provide any additional infomation compared to random selection.
...
The results show that the gini index is minimalised at $1<x<2$
...
```
Dataset 3 appears to be mushroom shaped, with a dense stem and a sparse cap upon inspection by tour. Using a correlation matrix shows that X1 and X2 could be two normal distributions with the same mean but different variances overlaid on one another, andthe variables X3, X4 and X5 all can be shown to be right skewed distributions with a certain degree of positive covariance, this results in the dense tight stem near the centre of the distributions, and the long tail fanning out into the cap of the mushroom. 
...
#tour
X11()
pal <- wes_palette("Darjeeling1", 3848, type = "continuous")
col <- pal[1:3848]
animate_xy(vischa[[5]], axes="bottomleft", col=col)
```

Using "Any technique", found the following by googling column names to identify the original synthesised dataset [3]
  ...
  While waiting for my tardy car-pool companion, I took a piece of graph paper, and figured out a dot-matrix representation of the word, "EUREKA." I then added these observations to the "center" of the datatset.
