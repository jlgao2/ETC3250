...
1. (5pts)This question is about the normal distribution, and how it relates to the classification rule provided by quadratic discriminant analysis.
   ...
   pnorm($p=1$, mean=$/mu_k$, sd=$/sigma_k$) 
...
y <- c(rep(x, n1), rep(x, n-n1))
df <- tibble(x, y)

x <- seq(-20, 30, by=1)
dx <- c(dnorm(x, m1, s1), dnorm(x, m2, s2))
y <- factor(c(rep(n1, length(x)), rep(n-n1, length(x))))
df_pop <- tibble(x=c(x,x), dx, y)
p <- ggplot() + 
  geom_line(data=n1, aes(x=x, y=0, colour=factor(y)), alpha=0.7) +
  geom_line(data=n2, aes(x=x, y=dx, colour="blue")) + 
...
score_gap <- scores %>% 
  group_by(CNT, gender) %>%
  summarise(math=weighted.mean(math, w=weight, na.rm=T),
            reading=weighted.mean(reading, w=weight, na.rm=T),
            science=weighted.mean(science, w=weight, na.rm=T)) %>%
...
cifn <- function(d, i) {
  x <- d[i,]
  ci <- weighted.mean(female, w=x$weight[x$gender=female], na.rm=T)-
                     weighted.mean(male, w=x$weight[x$gender=`male], na.rm=T)
  ci
}
bootfn <- function(d) {
...
score_gap_boot <- scores %>% 
  ...
  pivot_longer(cols=everything(), names_to="CNT", values_to="rank") %>%
  arrange(CNT) %>%
  mutate(bound=rep(c("ml","mu"), length(unique(scores$CNT)))) %>%
  pivot_wider(names_from = rank, values_from = bound)
score_gap <- score_gap %>%
...
ggplot(data=score_gap, aes(x=fct_reorder(name, math), y=math)) +
  ...
  geom_point() + 
  geom_linerange(aes(ymin=ml, ymax=mu), width=0) +
  coord_flip() + 
...
The data refers to the change of value of Australin Dollor relative US Dollor.
as for the picture, there was a little increase to 1.47 from Nov to Dec, which followed by a little decrease to Jan where it reached the nadir of 1.42. since then it begun to grow until mid-Mar and achieved the peak about 1.74, even though there was a small decline during the first quanrter reason of Mar from 1.55 to 1.5. however, the ratio decreased again from the period between mid-Mar and Apr.
...
ggplot(rates, aes(x=date, y=AUD))+geom_line()+ggtitle("Australian Dollar Currency Exchange")+xlab("month")+ylab("AUD relative to USD")
```
...
Australia Dollar, Canandian dollar, Swiss franc, Chinses Yuan, Euro, British pound, Indian rupee, Japanese yen, Korean won, Mexican peeo, New Zealand dollar, Russian ruble, Swedish krona, Singapore dollar, South African rand.
...
library(plotly)
rates_sub <- rates %>%
  select(c(date, AUD,CAD,CHF, CNY, EUR, GBP, INR, JPY, KRW, MXN, NZD, RUB, SEK, SGD, ZAR)) %>%
  mutate_if(rates_sub, function(x) -1*(x-mean(x))/sd(x))
rates_sub_long <- rates_sub %>% 
  pivot_longer(cols=-date, names_to="currency", values_to="count") 
ggplot(rates_sub_long, aes(x=date, y="count", colour= ???) + geom_line() +
...
d. (5)Conduct a principal component analysis on the subset of currencies. You need to work from a wide format of the data, where dates are in the columns, and currencies are in the rows. Normally, PCA operate on standardised variables but for this data, you need to NOT standardise each date. Think about why this is best.
    ...
    high dimensional data means the number of features more than the number of observations.In this case, the value of currencies or the change of currencies aganist USD with date exceed the observation.
   ...
   there are roughly 2 clusters, the one of EUR and JPY that both PC1 and PC2 explains around equally their most variance, the anther cluster of rest countries except CNY and CHF can be explained largely by PC1 not PC2, which behave similarly. 
...
A big movement happened sinc then the end of Feb.  
As for the picture, CNY,CHF behave strengthened relative to the USD.
Australia Dollar decreased relative USD during this period.
...
In this case, principal component analysis summarise the volatility of each currency aganist date. date dimension has been reduced.
    
```{r}
library(ggrepel)
rates_sub_wide <- rates_sub_long? %>%
  pivot_wider(id_cols=-date, names_from=date, values_from =count)
...
The statement is partly corrent.
PCA not only acts as a dimension reduction technique for regression by derivinh a low-dimensional set of features from a large set of variables; but also is a tool for data visualization or data pre-processing before supervised techniques by summarizing a smaller number of representive variables that explain most of the variability in the original set.
