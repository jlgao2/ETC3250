...
```
    e. If instead you had made a mistake and assumed that the two variances were equal, this would have produced a linear discrimant rule. Mark this boundary on the previous plot. Explain why and how this differs from result of the QDA rule.
      ...
      Using the linear discriminant rule instead of the quadratic discriminant rule in this example would result in worse classification performance as the discriminant rule only solves if the variance/covariance matrix is equal. If the assumption is false then it won't work. We can see in the example that clearing the QDA demarcates a better boundary as it accounts for different variances between categories in this univariate case. The samples here are overassigned to class 0 with LDA because it doesn't take into account
...
Write a paragraph explaining what you learn about the math gap across the countries tested in 2015. 
    ...
    The math gender gap varies widely in between countries with an unweighted average gap of 4.8 points, however girls outperform boys by 30 points in reading and 0.5 points in science. The gender gap in maths appears to be wider in Europe and South America in favour of boys and wider in favour of girls in Middle East and the Caribbean. 
...
Over the 5 month period the Australian dollar has weakened against the US dollar, with a big  decline in mid-March as the coronavirus impact affected the world. but australia in particular this time because of more interest rate cuts which encourage outflow of capital. There has also been supression of demand due to the early impact of coronavirus in china. The bushfires also reduced australian output. 
...
d. (5)Conduct a principal component analysis on the subset of currencies. You need to work from a wide format of the data, where dates are in the columns, and currencies are in the rows. Normally, PCA operate on standardised variables but for this data, you need to NOT standardise each date. Think about why this is best.
    ...
    - Why is this data considered to be high-dimensional?
      This data is high dimensional because in it's wide form the variables are the dates (each of which comprise a dimension),  and each observation is a currency and there are a whole lot more dates than currencies. We want to see the relationship between the currencies, to do this with PCA, we have to make the currencies the observations and the dates the variables 
    ...
    Write a summary of what you learn about the similarity and difference between the currencies.
      ...
      The majority of this basket appear to behave similarly as a group as per the most important PC with the notable exceptions of CNY, JPY, EUR and CHF. JPY and EUR are similar in PC1 and PC2, CNY and CHF appear to be relatively different. Opposing movements of the CNY and JPY in PC2 are seen in jan and feb inverse correlated movement on theplotly
      to be more volitile with some countercyclical movements. Particularly CHF not dropping much due to the 'rona and the CNY having a bit of 
    ...
    The loadings There was a big movement in PC1 started at around index 115 which corresponds to the end of Feburary. when the Rona really kicked in in the US and flattened out in Italy and Spain. The EUR and CHF really strengthened around 
    ...
    The variability in between currencies is being summarised in this PCA. 
...
**Principle component analysis is a dimension reduction technique**.
    ...
    Principle component analysis is both a dimension reduction technique but it is also a useful tool as applied here to understand the relationship between endogenous variables. We can use use the tool to more easily visualise higher dimension data. 
